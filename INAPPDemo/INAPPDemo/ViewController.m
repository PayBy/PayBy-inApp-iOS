...
- (void)viewDidLoad {
    ...
    placeOrderBtn.frame = CGRectMake(20, submitBtn.frame.origin.y+ 44 + 20, CGRectGetWidth(partnerId.bounds), 35);
    placeOrderBtn.backgroundColor = [UIColor redColor];
    [placeOrderBtn setTitle:@"place an order" forState:UIControlStateNormal];
    [placeOrderBtn setTitleColor:[UIColor whiteColor] forState:UIControlStateNormal];
    [self.view addSubview:placeOrderBtn];
    [placeOrderBtn addTarget:self action:@selector(placeOrderOfOnClick:) forControlEvents:UIControlEventTouchUpInside];
...
- (void)placeOrderOfOnClick:(UIButton *)sender {
    ...
    if(appIdTxt.text.length > 0 && partTxt.leftView > 0){
        ...
        [SDLPayByPaymentInterface paymentEnvironment:SDLPaymentEnvironmentDev];
        [SDLPayByPaymentInterface initInApp:appIdTxt.text partnerId:partTxt.text];
        ...
        [SDLPayByPaymentInterface payInAppWithViewContorller:self orderCallback:^(OrderSuccessCallback  _Nonnull orderSuccessCallback, OrderFailCallback  _Nonnull orderFailCallback) {
            //Simulation of getting order token asynchronously
            ...
            dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(3.0 * NSEC_PER_SEC)), queen, ^{
                ...
                dispatch_async(dispatch_get_main_queue(), ^(void) {
                    ...
                    if (token && token.length > 0) {
                        NSString *sigin = [NSString stringWithFormat:@"iapAppId=%@&iapDeviceId=%@&iapPartnerId=%@&token=%@",appIdTxt.text,deviceIdTxt.text,partTxt.text,token];
                        NSString *temgSig = [GPBRSA encryptString:sigin privateKey:privateKey];
                        orderSuccessCallback(token,deviceIdTxt.text,temgSig);
                    }else{
                        orderFailCallback();
                    }
                });
